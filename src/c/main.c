...
static void update_proc(Layer *layer, GContext *ctx) {
  ...
  for(int i=0; i<12; i++){
    if(i != 3) {
      // Plot dots
      dot_centre = (GPoint) {
        .x = (int16_t)(sin_lookup(TRIG_MAX_ANGLE * i / 12) * (int32_t)(s_dial_dots_radius) / TRIG_MAX_RATIO) + s_center.x,
        .y = (int16_t)(-cos_lookup(TRIG_MAX_ANGLE * i / 12) * (int32_t)(s_dial_dots_radius) / TRIG_MAX_RATIO) + s_center.y,
      };
  ...
  GPoint minute_hand = (GPoint) {
    .x = (int16_t)(sin_lookup(TRIG_MAX_ANGLE * mode_time.minutes / 60) * (int32_t)(s_gray_dial_radius) / TRIG_MAX_RATIO) + s_center.x,
    .y = (int16_t)(-cos_lookup(TRIG_MAX_ANGLE * mode_time.minutes / 60) * (int32_t)(s_gray_dial_radius) / TRIG_MAX_RATIO) + s_center.y,
  };
  GPoint hour_hand = (GPoint) {
    .x = (int16_t)(sin_lookup(hour_angle) * (int32_t)(s_gray_dial_radius) / TRIG_MAX_RATIO) + s_center.x,
    .y = (int16_t)(-cos_lookup(hour_angle) * (int32_t)(s_gray_dial_radius) / TRIG_MAX_RATIO) + s_center.y,
  };  

  // Draw hands
  graphics_context_set_fill_color(ctx, GColorRed);
  graphics_fill_circle(ctx, hour_hand, s_hand_radius);
  
  graphics_context_set_fill_color(ctx, GColorWhite);
  graphics_fill_circle(ctx, minute_hand, s_hand_radius);
  
  // Inner dial
  graphics_context_set_fill_color(ctx, GColorDarkGray);
  graphics_fill_circle(ctx, s_center, s_inner_dial_radius);
